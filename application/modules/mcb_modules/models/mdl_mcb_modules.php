...
class Mdl_MCB_Modules extends MY_Model {
	...
	private function refresh_core() {
		...
		$core_modules = directory_map(APPPATH . 'modules/', TRUE);
		...
		foreach ($core_modules as $core_module) {
			...
			$config_file = APPPATH . 'modules/' . $core_module . '/config/config.php';
			...
			if (file_exists($config_file)) {
				...
				*/
				...
				if ($query->num_rows()) {
					...
					$this->db->insert('modules', $db_array);
	...
	private function refresh_custom() {
		...
		$custom_modules = directory_map(APPPATH . 'modules/', TRUE);
		...
		foreach ($custom_modules as $custom_module) {
			...
			/* This should be the location of the module's config file */
			$config_file = APPPATH . 'modules/' . $custom_module . '/config/config.php';
