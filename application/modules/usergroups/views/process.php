...
<table style="width: 100%" class="tblgrid" cellpadding="0"
	...
	foreach ($groups as $group){
		...
		<tr <?php echo $trstyle;?>>
			...
			<td><a class="btn"
				onclick="showgroup('<?php echo $group->usergroup_id?>')"><?php echo icon("edit","Edit","png");?></a></td>
			<td style="text-align: center;"><a class="btn"><?php echo $status;?></a></td>
			<td><a class="btn"
				onclick="deletgroup('<?php echo $group->usergroup_id?>')"><?php echo icon("delete","Delete","png");?></a></td>
		</tr>
		<?php
		$i++;
	}
