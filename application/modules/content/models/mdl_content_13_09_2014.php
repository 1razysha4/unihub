...
class Mdl_Content extends MY_Model{
    ...
    function getPostsByUniversity($page,$university_id,$post_type='',$category_id = 0){
        ...
        $this->db->select('SQL_CALC_FOUND_ROWS c.content_id,c.content_name,c.content_date,c.content_type,ct.category_name,c.content_long_desc,u.username AS posted_by,c.content_created_ts,c.content_image,c.content_page_views,COALESCE(cm.total_comments,0) AS total_comments',false);
        $this->db->from($this->table_name.' AS c');
    ...
    function getUniversityDetailsByStates(){
    	...
    	;
		$sql = 'SELECT s.state_name,u.university_name,u.university_id,u.university_slug,COALESCE(c.total_posts,0)  AS total_posts,s.state_id,s.state_slug
				FROM ci_states AS s
				LEFT JOIN ci_universities AS u ON u.state_id=s.state_id  AND u.is_top=1
				LEFT JOIN (
